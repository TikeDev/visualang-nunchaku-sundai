...
Tasks:
Scaffold the full monorepo folder structure above
Initialize frontend with Vite + React using pnpm
Initialize backend with FastAPI
Install all dependencies (see below)
Build the Player component with:
...
Hardcode a sample data array of 4–6 entries with real image URLs (use any public placeholder images) and realistic timestamps
Confirm the player works end-to-end with hardcoded data before moving to Phase 2
...
Flow:
User pastes YouTube URL or uploads local audio file → UrlInput component
Frontend calls POST /transcript → gets transcript + audio path
Frontend calls POST /concepts → gets timestamped concepts + image prompts
Frontend calls POST /generate → gets image URLs with live progress updates
Preview player loads — hidden audio + Ken Burns images synced in browser
POST /export kicks off in background — FFmpeg renders the real .mp4
